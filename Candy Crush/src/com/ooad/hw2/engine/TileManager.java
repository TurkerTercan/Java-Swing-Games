...
public class TileManager extends JComponent implements Runnable{
    ...
    private static final Sprite exitButton = new Sprite((Helper.Window_Width - 250) / 2,  410, 251, 100, new ImageIcon(Objects.requireNonNull(TileManager.class.getResource(Helper.exitButtonPath))).getImage());
    private static final Sprite pauseButton = new Sprite( Helper.Window_Width - 80, 10, 70, 64, new ImageIcon(Objects.requireNonNull(TileManager.class.getResource(Helper.pauseButtonPath))).getImage());
    private static final Sprite gameOverImage = new Sprite( (Helper.Window_Width - 420) / 2, 190, 420, 240, new ImageIcon(Objects.requireNonNull(TileManager.class.getResource(Helper.gameOverPath))).getImage());
    private static final Sprite newGameButton = new Sprite((Helper.Window_Width - 400) / 2, 300, 400, 100, new ImageIcon(Objects.requireNonNull(TileManager.class.getResource(Helper.newGameButtonPath))).getImage());
    ...
    private ArrayList<Integer> x_array = new ArrayList<>();
    private ArrayList<Integer> y_array = new ArrayList<>();
    ...
    private final Collidable[] enemies = new Collidable[3];
    private final Collidable[] characters = new Collidable[3];
    ...
    public TileManager() {
        board = new Sprite((Helper.Window_Width - 700) / 2, Helper.tileLocationY, 700, 452,
                new ImageIcon(Objects.requireNonNull(getClass().getResource(Helper.tileBoardPath))).getImage());
        logBoard = new Sprite(0, Helper.Window_Height - 170, 700, 452,
                new ImageIcon(Objects.requireNonNull(getClass().getResource(Helper.logBoardPath))).getImage());
    ...
    public void onDraw(Graphics2D g2D) {
        ...
        g2D.drawString(Logger.getFirstLog(), 50, Helper.Window_Height - 115);
        g2D.drawString(Logger.getSecondLog(), 50, Helper.Window_Height - 85);
        g2D.drawString(Logger.getThirdLog(), 50, Helper.Window_Height - 55);
        g2D.drawString(Logger.getForthLog(), 50, Helper.Window_Height - 25);
    ...
    public void run() {
        ...
        while (isRunning) {
            ...
            if (isAnimationRunning && fifth_step) {
                finalCheckandChangeTurn();
            }
        }
    }

    /**
     * Checks for any other tile matches. If there is runs the animations and creates new tiles again.
     * If there isn't, makes the computer one turn.
     */
    private void finalCheckandChangeTurn() {
        isAnimationRunning = false;
        fifth_step = false;
        first_step = true;
        animMap.clear();
    ...
    /**
     * The new created tiles will be the below of the screen and this method will make those tiles to fit their place like animation.
     */
    private void newCreatedTilesAnim() {
        int count = 0;
    ...
    /**
     * After the matched tiles are thrown, there will be empty holes that tiles that can fit.
     * This method makes the tiles move upwards if there is a hole.
     */
    private void ScrollUpToEmptyTileAnim() {
        for (int j = 0; j < tiles[0].length; j++) {
        ...
        if (temp_count != 0) {
            Logger.setSingleLog(temp_count + " tiles have been matched.");
        }
    ...
    /**
     * This method will make the matched tiles move upwards until they reach to a character or an enemy.
     */
    private void throwMatchedTilesAnim() {
            ...
            if (player_turn) {
                if (tiles[x_array.get(i)][y_array.get(i)].getSprite().getY() < Helper.tileLocationY) {
                    ...
                    if (enemies[y_array.get(i) / 3] != null && enemies[y_array.get(i) / 3].isDead()) {
                        Logger.setSingleLog(enemies[y_array.get(i) / 3].toString() + " is dead.");
    ...
    private void switchTileAnimation() {
    ...
    public static void checkForTiles(int x, int y) {
        if (pauseButton.mouseDetectionForMouseInput(x, y) && !gameover) {
            pauseFlag = !pauseFlag;
            if (!pauseFlag)
                Logger.setSingleLog("Unpause");
            else
                Logger.setSingleLog("Pause");
        } else if ((pauseFlag || gameover) && newGameButton.mouseDetectionForMouseInput(x, y)) {
            resetSignal = true;
            Logger.setSingleLog("New game started");
